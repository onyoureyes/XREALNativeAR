...
Java_com_xreal_nativear_MainActivity_initQNN(JNIEnv *env, jobject /* this */,
  ...
  const char *libPath = env->GetStringUTFChars(nativeLibPath, nullptr);
  
  LOGI("Initializing QNN with model: %s", path);
  LOGI("Native Library Path: %s", libPath);

  try {
    // 1. Initialize QNN Runtime
    g_qnnRuntime = new QNNRuntime();
    // Pass the library path to initialize() for ADSP_LIBRARY_PATH
    if (!g_qnnRuntime->initialize(libPath)) {
      LOGE("Failed to initialize QNN Runtime");
      env->ReleaseStringUTFChars(modelPath, path);
      env->ReleaseStringUTFChars(nativeLibPath, libPath);
      return JNI_FALSE;
    }
