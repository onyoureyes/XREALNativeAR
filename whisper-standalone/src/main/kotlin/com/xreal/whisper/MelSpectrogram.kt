...
class MelSpectrogram {
    ...
    // Whisper-Tiny Mel Scaling Constants:
    // Whisper expects the mel spectrogram to be normalized between [-1.0, 1.0].
    // This is achieved by taking max(log10(mels)) and clipping to 8dB dynamic range.
    ...
    fun process(audioData: ShortArray): FloatArray {
        ...
        // 1. Audio Transformation & Padding:
        // Convert ShortArray PCM to 16-bit Float and pad to 30s.
        // Whisper handles fixed 30s chunks.
        val audioFloats = FloatArray(480000 + N_FFT) 
        ...
        for (frame in 0 until numFrames) {
            ...
            // C. Power Spectrum calculation (|X|^2)
            // JTransforms complex output format: [Re0, Im0, Re1, Im1, ...]
            ...
            // D. Mel Filterbank Matrix Multiplication:
            // Projects the 201 power spectrum bins into 80 Mel frequency bins.
            for (m in 0 until N_MELS) {
                ...
                // E. Log Compression (Log-Mel)
                val logMel = log10(max(sum, 1e-10f))
                
                // F. Flatten for Whisper Model:
                // Maps the 2D mel-frame [m, frame] to a 1D array.
